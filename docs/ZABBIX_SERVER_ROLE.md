...
* `zabbix_server_mysql_auth_plugin`: Allows selecting MySQL authentication plugin (`mysql_native_password`, `caching_sha2_password`, `sha256_password`)
* `zabbix_server_mysql_auth_salt`: Required for idempotency when creating a MySQL user using `caching_sha2_password` or `sha256_password` authentication plugins. Has to be exactly 20 characters
